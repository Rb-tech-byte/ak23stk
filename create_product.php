...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $slug && $price && $stock_quantity !== false && $category_id && empty($_SESSION['error_message'])) {
        try {
            $stmt = $pdo->prepare("INSERT INTO products (name, slug, description, price, stock_quantity, category_id, image_url) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $slug, $description, $price, $stock_quantity, $category_id, $target_file]);
